...
read_plus <- function(flnm) {
  ...
  if(grepl('dynasty-overall|rookies',flnm))
  {vecNames <- c('Rank','a1','Player','Pos','Bye','Age','Best','Worst','Avg','SD','Notes')}
  else if(grepl('dst|cheatsheets',flnm))
  {vecNames <- c('Rank','a1','Player','Bye','Best','Worst','Avg','SD','Notes')}
  #else if(grepl('cheatsheets',flnm))
  #{names(df) <- c('Rank','a1','Player','Bye','Best','Worst','Avg','SD','Notes')}
  else
  {vecNames <- c('Rank','a1','Player','Bye','Age','Best','Worst','Avg','SD','Notes')}
  
  df <- read.csv(flnm, col.names = vecNames, sep = ',') 
  
  
  numCols <- c("Rank","Bye","Best","Worst","Avg","SD")#,"ADP","vs, ADP")
