...
server <- function(input, output, session) {
  ...
  franchises<-fromJSON(paste0("https://www03.myfantasyleague.com/2019/export?TYPE=league&L=",reactive(input$franchiseid),"&APIKEY=&JSON=1"))$league$franchises$franchise %>%
    select(ownerid=id,owner=name)
  
  standings<-fromJSON(paste0("https://www03.myfantasyleague.com/2019/export?TYPE=leagueStandings&L=",reactive(input$franchiseid),"&APIKEY=&JSON=1"))$leagueStandings$franchise %>% 
  ...
  schedule <-fromJSON(paste0("https://www03.myfantasyleague.com/2019/export?TYPE=schedule&L=",reactive(input$franchiseid),"&APIKEY=&JSON=1"))$schedule$weeklySchedule %>% 
    unnest(matchup) %>% 
